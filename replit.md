...
- `/portfolio/benchmarks`: a linha "Carteira" usa a rentabilidade real atual (mesmo cálculo do `/portfolio/summary`), só que **fixa nos 12 pontos** — não temos snapshot histórico de patrimônio pra reconstruir a evolução mês a mês, então não inventa uma curva. Carteira vazia mostra 0% reto, não um ganho fake (bug corrigido — antes era `Math.random()` puro, nem consultava os ativos do usuário). CDI/IBOV/IFIX continuam simulados, sem fonte real conectada ainda.
