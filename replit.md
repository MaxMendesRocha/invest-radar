...
- Motor de análise (`artifacts/api-server/src/lib/analysis-engine.ts`) é **regras determinísticas**, não IA/LLM — nenhuma chamada de modelo de linguagem envolvida. Usa fundamentos reais da brapi.dev (`getFundamentals`, endpoint `?modules=defaultKeyStatistics,financialData`): P/L, P/VP, ROE, dívida/patrimônio, margem líquida, DY, crescimento de receita, variação 12m e beta. Pesos seguem a fórmula do "Score do Radar" do prompt original (Fundamentos 40%, Notícias 20%, Macro 20%, Tendência 10%, Volatilidade 10%) — Notícias/Macro ainda não têm fonte de dados real (Fase 3), então entram com score neutro (60) em vez de inventados; Tendência (variação 12m) e Volatilidade (beta) já são reais. Categorias sem ticker de bolsa (`renda_fixa`/`fundos`) recebem um resultado neutro fixo, sem tentar buscar fundamentos. `newsItems` sempre vem vazio até a Fase 3 — nunca fabricar manchete falsa ali.
