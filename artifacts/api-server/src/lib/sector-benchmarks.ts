...
/**
 * Leitura barata das médias setoriais já persistidas por regenerateOpportunities()
...
// Só compara os fundamentos que ambos os lados têm reais (ativo e média do setor) —
...
export function describeSectorComparison(f: Fundamentals, benchmark: SectorBenchmark | null): string {
  ...
  if (f.priceEarnings != null && peBenchmark != null && peBenchmark > 0) {
    ...
    lines.push(`P/L de ${f.priceEarnings.toFixed(1)} vs. média do setor de ${peBenchmark.toFixed(1)} (${diff >= 0 ? "+" : ""}${diff.toFixed(0)}%)`);
  }
  ...
  if (f.returnOnEquity != null && roeBenchmark != null) {
    ...
    lines.push(`ROE de ${(f.returnOnEquity * 100).toFixed(1)}% vs. média do setor de ${(roeBenchmark * 100).toFixed(1)}% (${diff >= 0 ? "+" : ""}${diff.toFixed(0)}%)`);
  }
  ...
  if (f.dividendYield != null && dyBenchmark != null) {
    ...
    lines.push(`DY de ${(f.dividendYield * 100).toFixed(1)}% vs. média do setor de ${(dyBenchmark * 100).toFixed(1)}% (${diff >= 0 ? "+" : ""}${diff.toFixed(0)}%)`);
  }
