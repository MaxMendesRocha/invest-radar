...
function computeAnalysis(
  ...
  return fundamentals
    ? analyzeFundamentals(fundamentals, dps12mByTicker.get(upper) ?? null, positionPercentByTicker.get(upper) ?? 0)
    : pendingAnalysis();
}
...
router.get("/analysis/assets", requireAuth, async (req, res): Promise<void> => {
  ...
  const [fundamentalsByTicker, { dps12mByTicker, dividendFrequencyByTicker }, positionPercentByTicker] = await Promise.all([
  ...
  const result = await Promise.all(
    assets.map(async (asset) => {
      ...
      return toApiShape(asset.ticker, computeAnalysis(asset.ticker, asset.category, fundamentalsByTicker, dps12mByTicker, positionPercentByTicker), newsItems, dividendFrequency);
    })
  );
...
router.get("/analysis/assets/:ticker", requireAuth, async (req, res): Promise<void> => {
  ...
  const dividendFrequency = dividendFrequencyByTicker.get(asset.ticker.toUpperCase()) ?? null;
  res.json(toApiShape(asset.ticker, computeAnalysis(asset.ticker, asset.category, fundamentalsByTicker, dps12mByTicker, positionPercentByTicker), newsItems, dividendFrequency));
});
...
router.post("/analysis/generate", requireAuth, async (req, res): Promise<void> => {
  ...
  const analyses = assets.map((a) => ({
    ...
    ...computeAnalysis(a.ticker, a.category, fundamentalsByTicker, dps12mByTicker, positionPercentByTicker),
    taxEstimate: null as TaxEstimate | null,
    technical: null as TechnicalIndicators | null,
